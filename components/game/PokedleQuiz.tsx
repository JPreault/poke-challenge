...
const POKEDEX_COLUMNS = ["Pokémon", "Génération", "Type 1", "Type 2", "Habitat", "Couleur(s)", "Stade d'évolution", "Hauteur", "Poids"] as const;
...
function hintCellClass(status: AttemptHints[keyof AttemptHints]) {
    return cn(
        ...
        status === "partial" && "border-amber-300 bg-amber-50 text-amber-800 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-300",
        status === "wrong" && "border-poke-red/40 bg-poke-red/5 text-poke-red dark:bg-poke-red/10",
    );
}
    ...
    const handleSkip = useCallback(async () => {
        ...
        try {
            ...
            session.recordRound({
                ...
                correctImage: result.targetArtworkUrl,
                hintAccuracyPercent: lastAttempt ? getHintAccuracyPercent(lastAttempt.hints) : 0,
            });
        ...
        <>
            <div className={hintCellClass(isCorrect ? "correct" : "wrong")}>
                <div className="flex h-full items-center justify-center">
                    <div className="relative h-16 w-16">
                        <Image src={attempt.spriteUrl} alt={attempt.nameFr} fill sizes="64px" unoptimized className="object-contain" />
                    </div>
                </div>
            </div>
            <div className={hintCellClass(hints.generation)}>
                <div className="flex h-full items-center justify-center gap-1">
                    ...
                    {hints.generation !== "correct" ? <span className="text-base">{getDirectionSymbol(directions.generation)}</span> : null}
                </div>
            </div>
            ...
            <div className={hintCellClass(hints.habitat)}>
                <div className="flex h-full items-center justify-center">{attempt.habitat ?? "Aucun"}</div>
            </div>
            ...
            <div className={hintCellClass(hints.evolutionStage)}>
                <div className="flex h-full items-center justify-center gap-1">
                    ...
                    {hints.evolutionStage !== "correct" ? <span className="text-base">{getDirectionSymbol(directions.evolutionStage)}</span> : null}
                </div>
            </div>
            <div className={hintCellClass(hints.heightM)}>
                <div className="flex h-full items-center justify-center gap-1">
                    <span>{`${attempt.heightM} m`}</span>
                    {hints.heightM !== "correct" ? <span className="text-base">{getDirectionSymbol(directions.heightM)}</span> : null}
                </div>
            </div>
            <div className={hintCellClass(hints.weightKg)}>
                <div className="flex h-full items-center justify-center gap-1">
                    <span>{`${attempt.weightKg} kg`}</span>
                    {hints.weightKg !== "correct" ? <span className="text-base">{getDirectionSymbol(directions.weightKg)}</span> : null}
                </div>
            </div>
        </>
    );
