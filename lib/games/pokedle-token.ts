...
export interface PokedlePayload {
  targetId: number;
  exp: number;
}
...
export function createPokedleToken(targetId: number): string {
...
export function verifyPokedleToken(token: string): PokedlePayload | null {
  ...
  try {
    ...
    const payload = JSON.parse(decrypted.toString("utf8")) as PokedlePayload;
