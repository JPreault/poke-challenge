...
import { buildShuffleGamesQuery, createShuffleDeck, drawNextShuffleRoundType } from "@/lib/games/shuffle";
import {
...
export function ShuffleQuiz({ session, selectedRoundTypes, useBacPool = true, interfaceMode = "arena" }: ShuffleQuizProps) {
    const isRanked = interfaceMode === "ranked";
    const ranked = useRankedSession();
    const effectiveRoundTypes = isRanked ? ARENA_SHUFFLE_ROUND_TYPES : selectedRoundTypes;
    ...
    const drawRound = useCallback(() => {
        const { nextType, remainingDeck } = drawNextShuffleRoundType(deckRef.current, selectedRef.current);
        deckRef.current = remainingDeck;
        if (isRanked) {
            rankedRef.current?.setRoundAttemptLimit(getRankedAttemptLimit(nextType) ?? 1);
        }
    ...
    const roundDescription = roundType ? getShuffleRoundDescription(roundType) : "Un mini-jeu aléatoire à chaque manche.";
    ...
    const homeHref = isRanked ? "/partie-classee" : interfaceMode === "bac-training" ? "/entrainement" : "/";
    const replayHref = isRanked ? "/game/shuffle?interface=ranked" : `/game/shuffle?${replayParams.toString()}`;
    ...
    >
            {!roundType ? (
                <div className="flex h-64 items-center justify-center text-muted-foreground">Préparation de la manche…</div>
            ) : (
                <div key={roundKey}>
                    {roundType === "image-to-name" ? (
                        <ImageToNameRound session={session} onRoundComplete={drawRound} useBacPool={resolvedUseBacPool} />
                    ) : null}
                    {roundType === "name-to-image" ? (
                        <NameToImageRound session={session} onRoundComplete={drawRound} useBacPool={resolvedUseBacPool} />
                    ) : null}
                    ...
                    {roundType === "cry-guess" ? <CryGuessRound session={session} onRoundComplete={drawRound} /> : null}
                    {roundType === "pokedle" ? <PokedleRound session={session} onRoundComplete={drawRound} /> : null}
                    {roundType === "description-guess" ? <DescriptionGuessRound session={session} onRoundComplete={drawRound} /> : null}
                    {roundType === "blur-guess" ? (
                        <BlurGuessRound session={session} onRoundComplete={drawRound} useBacPool={resolvedUseBacPool} />
                    ) : null}
                    {roundType === "zoom-guess" ? (
                        <ZoomGuessRound session={session} onRoundComplete={drawRound} useBacPool={resolvedUseBacPool} />
                    ) : null}
                </div>
            )}
        </GameShell>
